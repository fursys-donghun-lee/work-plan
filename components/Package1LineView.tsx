...
export function Package1LineView() {
  ...
  const setOverride = useDataStore((s) => s.setPackageWorkerOverride);
  const clearOverride = useDataStore((s) => s.clearPackageWorkerOverride);
  const resetOverrides = useDataStore((s) => s.resetPackageWorkerOverrides);
  const supportPlacements = useDataStore((s) => s.packageSupportPlacements);
  const setSupportPlacement = useDataStore((s) => s.setPackageSupportPlacement);
  const clearSupportPlacement = useDataStore((s) => s.clearPackageSupportPlacement);
  const groupMerges = useDataStore((s) => s.packageGroupMerges);
  const addGroupMerge = useDataStore((s) => s.addPackageGroupMerge);
  const removeGroupMerge = useDataStore((s) => s.removePackageGroupMerge);
  const workDate = useDataStore((s) => s.workDate);

  // 다른 라인에서 들어온 지원 인원 = supportAssignments 의 confirmed 합 (targetLine = 포장1라인)
  ...
  // 다른 회사 그룹들의 supportablePeople 통합 (포장1라인이 받는 confirmed 계산용)
  // 본인 package1 그룹은 굳이 포함 안 해도 됨 (포장1라인 → 포장1라인 받기는 없음)
  ...
  if (missing.length > 0) {
    return (
      <EmptyState
        ...
        description={`현재 미업로드: ${missing.join(", ")}\n\n포장1라인 부하는 포장라인 기본근무위치(기준자료) + 근태 + 라인별 포장 부하(일일자료)가 필요합니다.`}
        ctaLabel={
    ...
    <div className="space-y-4">
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-3">
          <div>
            <h2 className="font-semibold text-slate-900">포장1라인 전체</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-3">
        ...
        {groupMerges.map((merge, mIdx) => {
          const mergedRows = merge
            ...
            .filter((r): r is Package1GroupLoad => !!r);
...
type MergedGroupLoad = Omit<Package1GroupLoad, "group"> & { group: string };

function round1(n: number): number {
  return Math.round(n * 10) / 10;
}

function mergeGroupLoads(rows: Package1GroupLoad[]): MergedGroupLoad {
